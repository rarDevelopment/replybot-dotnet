...
public class InternetGameDatabaseApi(IGDBClient igdbClient)
{
    ...
    {
        var queryFields = $"fields id,name,cover,release_dates.*,release_dates.status.*,platforms.*,status,url,websites,release_dates.release_region.*; search \"{searchTerm}\"; where version_parent = null & (category = 0  | category = 8);";
