...
public class GuildConfigurationBusinessLayer(IReplyDataLayer replyDataLayer) : IGuildConfigurationBusinessLayer
{
    ...
    public async Task<bool> SetEnableRepeatLinkNotifications(IGuild guild, bool isEnabled)
    {
        GuildConfiguration? config = await replyDataLayer.GetConfigurationForGuild(guild.Id.ToString(), guild.Name);
        if (config != null)
        {
            return await replyDataLayer.SetEnableRepeatLinkNotifications(guild.Id.ToString(), isEnabled);
        }
    ...
    public async Task<bool> SetEnableDepartureMessage(IGuild guild, bool isEnabled)
    {
        GuildConfiguration? config = await replyDataLayer.GetConfigurationForGuild(guild.Id.ToString(), guild.Name);
        if (config != null)
        {
            return await replyDataLayer.SetEnableDepartureMessage(guild.Id.ToString(), isEnabled);
        }
